 ...
 */
function viking_wp_title( $title, $sep ) {
	global $page, $paged;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'viking' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'viking_wp_title', 10, 2 );
...
function viking_continue_reading( $id ) {
	return '<a href="'. get_permalink( $id ). '">' . __( 'Continue reading: ', 'viking') . get_the_title($id). '</a>';
}
